...
def get_stat_values(info_file):
  ...
  for i in range(len(outlist)-1):
    if outlist[i][0] != '#':
      stat_name,stat_lat,stat_lon,stat_ev = outlist[i].split(' ')
      lat_deg,lat_dec = (str(stat_lat)).split('.')
      lon_deg,lon_dec = (str(stat_lon)).split('.')
      #convert decimal degrees into minutes
      minutes_lat = '%05.2f' % (float('0.'+lat_dec)*60.)
      minutes_lon = '%05.2f' % (float('0.'+lon_dec)*60.)
...
def write_pha(out):
  ...
  '''
  import os
  if not os.path.isfile('eq.pha'):
    outfile = open('eq.pha','w')
  else:
    outfile = open('eq.pha','a')
  n = 1
  #Event line
  outfile.write('# %4i %02i %02i %02i %02i %05.2f %8.4f %8.4f %5.2f 0.0 0.0 0.0 0.0' % (out['origin_time'].year,out['origin_time'].month,out['origin_time'].day,out['origin_time'].hour,out['origin_time'].minute,(out['origin_time'].second + (out['origin_time'].microsecond/1e6)),float(out['ev_lat']),float(out['ev_lon']),float(out['ev_depth'])))
  outfile.write(' '+str(n)+'\n') 
  #Pick lines
  for j in out['Tobs']:
    outfile.write(j.replace('_',' ')+' '+out['Tobs'][j]+' 1.00 P\n')
    if out['TobsS'].has_key(j):
